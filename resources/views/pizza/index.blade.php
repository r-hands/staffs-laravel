            ...
            @foreach ($pizza as $pizza)
                <div class="p-6 flex space-x-2">
                    ...
                    <div class="flex-1">
                            ...
                            <div>
                                ...
                                @if ($pizza->user->is(auth()->user()))
                                    <x-dropdown>
                                        ...
                                        <x-slot name="content">
                                            <x-dropdown-link :href="route('pizza.edit', $pizza)">
                                                {{ __('Edit') }}
                                            </x-dropdown-link>
                                            <form method="POST" action="{{ route('pizza.destroy', $pizza) }}">
                                            @csrf
                                            @method('delete')
                                            <x-dropdown-link :href="route('pizza.destroy', $pizza)" onclick="event.preventDefault(); this.closest('form').submit();">
                                                {{ __('Delete') }}
                                            </x-dropdown-link>
                                        </form>
                                        </x-slot>
                        ...
                        <p class="mt-4 text-lg text-gray-900">{{ $pizza->message }}</p>
                    </div>
                </div>
            @endforeach
        </div>
